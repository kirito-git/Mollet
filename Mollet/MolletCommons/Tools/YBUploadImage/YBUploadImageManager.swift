...
class YBUploadImageManager: NSObject, UIActionSheetDelegate, UINavigationControllerDelegate,UIImagePickerControllerDelegate {
    ...
    var uploadImageClourse:UploadImageToServerClourse?
    
    public weak var fatherViewController: UIViewController?
    ...
    func showActionSheetInFatherViewController(fatherVC:UIViewController,  uploadClourse: @escaping UploadImageToServerClourse) {
        ...
        let acctionSheet: UIActionSheet =  UIActionSheet.init(title: nil, delegate: self, cancelButtonTitle: "Cancel", destructiveButtonTitle: nil, otherButtonTitles: "Take Photo", "Select from album")
   ...
   func createPhotoView() {
        ...
        if isAvailable {
            ...
            let alert = UIAlertView.init(title: "Warning!", message: "Camera unavailable!", delegate: nil, cancelButtonTitle: "Confirm")
            alert.show()
        }
   ...
   //从图片库中选择
    ...
    func imagePickerController(_ picker: UIImagePickerController, didFinishPickingMediaWithInfo info: [String : Any]) {
        ...
        self.fatherViewController?.navigationController?.dismiss(animated: true, completion: {
            ...
            //图片返回
            print(image)
            if self.uploadImageClourse != nil {
                self.uploadImageClourse!(image, "")
            }
